...
public class UserDAOImpl implements UserInfoDAO {
	...
	public UserInfoVO selectUser(UserInfoVO user) {
		String sql = "select ui_num,ui_name,ui_num, ui_age, ui_birth,\r\n" + 
	            ...
	            "ui_credat,ui_nickname,ui_credat from user_info where ui_num = ?";
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		try {
			con = InitServlet.getConnection();
			ps = con.prepareStatement(sql);
			ps.setNString(1, user.getUiId());
			ps.setNString(2, user.getUiPassword());
			rs = ps.executeQuery();
	...
	public static void main(String[] args) {
		...
		is.init();
		UserInfoDAO usDao = new UserDAOImpl();
		UserInfoVO user = new UserInfoVO();
		user.setUiId("test");
		user.setUiPassword("test");
		user=usDao.selectUserForLogin(user);
